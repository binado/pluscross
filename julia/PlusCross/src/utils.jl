...
function frequency_array(;
    ...
    sampling_frequency > 0 || throw(
        ArgumentError("sampling_frequency must be positive, got $(sampling_frequency)")
    )
    duration > 0 || throw(ArgumentError("duration must be positive, got $(duration)"))
    nyquist_frequency = sampling_frequency / 2
    0 <= minimum_frequency <= maximum_frequency <= nyquist_frequency || throw(
        ArgumentError(
            "expected 0 <= minimum_frequency <= maximum_frequency <= nyquist_frequency ($(nyquist_frequency)), got minimum_frequency=$(minimum_frequency), maximum_frequency=$(maximum_frequency)",
        ),
    )
    ...
    abs(n_samples_exact - n_samples) <= tolerance || throw(
        ArgumentError(
            "duration * sampling_frequency must be an integer number of samples, got duration=$(duration), sampling_frequency=$(sampling_frequency) ($(n_samples_exact) samples)",
        ),
    )
    iseven(n_samples) || throw(
        ArgumentError(
            "duration * sampling_frequency must be an even integer so the frequency grid reaches the Nyquist frequency exactly, got $(n_samples) samples (duration=$(duration), sampling_frequency=$(sampling_frequency))",
        ),
    )
